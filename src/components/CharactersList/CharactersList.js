...
import { getfilteredByName } from 'redux/operations';
import { getFilteredCharacters, getName } from 'redux/selectors';
...
const CharactersList = () => {
  ...
  let count = characters.length >= 20;

  useEffect(() => {
    setPage(2);
    if (characters.length <= 10) {
      Notify.info(`We found only ${characters.length} results`);
    }
  }, [characters.length, name]);
  const baseImg =
      ...
      <Grid>
        {[...characters]
          ...
          .map(({ id, image, name, species }) => (
            <GridItem key={id}>
              <Link
                to={`/character/${decodeURIComponent(
                  name
                    .toLowerCase()
                    .trim()
                    .split(' ')
                ...
                state={{ from: location }}
              >
                <StyledImg src={image ? image : baseImg} alt={name} />
              </Link>
      ...
      {count && (
        <Button
          onClick={() => {
            ...
            dispatch(getfilteredByName({ name, page }));
            window.scrollTo(0, 0);
          }}
        />
      )}
